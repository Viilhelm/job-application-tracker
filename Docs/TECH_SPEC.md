...
`Company` is a rollup through the relation, not a copied value: the company belongs to the application, and a second stored copy would disagree the moment the application is edited. Both rollups are added on the first save rather than at creation, because a rollup cannot reference a relation defined in the same request. **Add missing Notion properties** in Settings runs the same checks on demand, so a convenience column introduced by a newer version does not wait for the next saved message.
